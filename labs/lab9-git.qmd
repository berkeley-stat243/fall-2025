...
The diagram below comes from the book "Research Software Engineering with Python", [Chapter 6 - Using Git at the Command Line](https://third-bit.com/py-rse/git-cmdline.html#git-cmdline-remotes) and demonstrates the interplay of the concepts above.

![Commands included in basic Git workflows (Credit: Research Software Engineering with Python, licensed under CC-BY 4.0).](lab9-git/git-remote.png)
...
3. Each person create a new branch (call them something different), add a file
  ...
  either merge on the command line or use a pull request. (Make sure to use `git
  pull` before pushing your merge to the remote repo if you do it through the
  command line to avoid merge conflicts.)
