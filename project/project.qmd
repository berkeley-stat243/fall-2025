...
7. Your code must pass the tests in [this template repository](foo) when run via GitHub Actions, and it should produce reasonable assessment output there as well. (The action will run `test.py` via `pytest` and `assess.py` via `python assess.py`.) Then when I am grading the project, I will have an expanded test.py and assess.py that I will use as a large part of the grade. This will run your GA on some additional datasets. This is not a bake-off, so I am not grading based on you achieving the absolute best predictor selection or MSPE results, but I do expect good performance and reasonable run-times. The results of `assess.py` will provide guidance to you.

[discuss non simple linear regression]
For some prediction methods, regularization is built in and the methods may essentially ignore some of the potential predictors. This means that the fitness score, even when based on CV, may not penalize non-sparse solutions. For such situations, the `penalty` argument is provided. In my grading I'll use this argument (set to some "moderate" value that aims to produce reasonably sparse solutions when there are a limited set of predictors that really influence the outcome).
...
3. You should use Git and GitHub to manage your collaboration. Your repository should be a private repository named `GA-dev` either be at github.com or github.berkeley.edu
