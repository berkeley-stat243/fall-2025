...
## response = requests.get(URL)   # Now getting a 403 error.
## data = xmltodict.parse(response.content)
## Download to newest.xml file manually.
...
search (using `requests`), where here we will create a GitHub issue
...
```
Note that for security, I created a [GitHub personal access token](https://github.com/settings/personal-access-tokens) that is limited in *scope* to
only be able to handle issues in my test repository.
