...
-   Irving et al. [Research Software Engineering with Python](https://third-bit.com/py-rse/)
...
Use an editor (or integrated development environment) that supports the language you are using (e.g., *VS Code*, *Atom*, *Emacs*/*Aquamacs*, *Sublime*, *vim*, *TextMate*, *WinEdt*, or the built-in editor in *RStudio* [you can use Python from within RStudio]). Some advantages of this can include:
...
Check that the file `missing_file.txt` can be found in the current path: `/accounts/vis/paciorek/teaching/243/fall-2025/units`.
```
...
rootdir: /accounts/vis/paciorek/teaching/243/fall-2025/units
plugins: anyio-4.3.0
collected 4 items                                                                               
...
There is now a large variety of AI-assisted coding tools that available within integrated development environment and allow you to easily work with AI while you are coding. This makes for an easier workflow than copy-pasting from a ChatBot.

:::{.callout-warning title="Moving fast!"}
...
- AI-assisted code suggestions (accept/reject like grammar suggestions in text editing)
- Built-in Chat window
- Editing/Agent capabilities where a user can ask the tool to change or add to their code based on a text prompt, or potentially generate a whole program/application.
- Ability to give files/directories as context. 

Many of the tools are built on top of, or integrated into, VS Code (e.g., Cursor, GitHub Copilot).
...
A potential hierarchy of uses:
 ...
 - For straightforward syntax where checking the outcome is sufficient (e.g., plot formatting code).
 - For involved small pieces of code where extensive testing may be sufficient (e.g., regular expressions).
...
```
>>> run.fit(data, n_cats)
> /accounts/vis/paciorek/teaching/243/fall-2025/units/run_with_break.py(10)fit()
...
```
(Pdb) n
> /accounts/vis/paciorek/teaching/243/fall-2025/units/run_with_break.py(11)fit()
...
```
(Pdb) u
> /accounts/vis/paciorek/teaching/243/fall-2025/units/run_no_break.py(10)fit()
-> model = statsmodels.api.OLS(sub['y'], statsmodels.api.add_constant(sub['x']))
```
...
```
> /accounts/vis/paciorek/teaching/243/fall-2025/units/run_no_break2.py(1)<module>()
-> import numpy as np
(Pdb) 
```
...
```
(Pdb) b 9
Breakpoint 1 at /accounts/vis/paciorek/teaching/243/fall-2025/units/run_no_break.py:9
```

```{python}
#| eval: false
(Pdb) c
> /accounts/vis/paciorek/teaching/243/fall-2025/units/run_no_break2.py(9)fit()
-> model = statsmodels.api.OLS(sub['y'], statsmodels.api.add_constant(sub['x']))
```
...
```
(Pdb) disable 1
Disabled breakpoint 1 at /accounts/vis/paciorek/teaching/243/fall-2025/units/run_no_break2.py:9
(Pdb) b fit
Breakpoint 1 at /accounts/vis/paciorek/teaching/243/fall-2025/units/run_no_break.py:6
```
