...
Object protocols are implemented based on the [Python object model](#the-python-object-model-and-dunder-methods) using **dunder** methods.
...
The answer depends on where `f` is defined. If it is defined in a nested fashion within another function (or functions),
then the lookup occurs in the (enclosing) scope(s) of those functions.
...
def number_formatter(notation='US'):
    ...
    def format_number(number):
        ## GitHub Copilot suggested the `number:,` syntax and the string replace method.
        result = f"{number:,}"
        if notation == 'US':
