...
All objects that fall in a given category share key characteristics. For example `sequence` objects have notions of length and accessing (ordered) elements by index, while
`iterator` objects have notions of "next" and of "stopping".
...
Here we see that `tuple`s are iterable containers (though they are not iterators themselves):
...
Side note: `range` objects can be iterated over (they are iterable) but they are not actually iterators. E.g., looping over them does not "consume" their values, unlike an iterator. 
...
With either approach, we get (1) exception handling in case something goes wrong with the read/write (in this case `readlines`) and (2) automatic closing of the file, regardless of what happens in the execution of the code.

What does this have to do with *dunder* methods?

Well, one can use `with` with any class that provides `__enter__` and `__exit__` methods. These are invoked before and after the code in the scope of the `with` block is run.
