...
:::{.callout-warning title="Simulation (Monte Carlo) uncertainty is not statistical uncertainty"}
...
uFromR <- runif(10)                    # Use built-in RNG

## Set up manual RNG.
a <- c(171, 172, 170)
m <- c(30269, 30307, 30323)
u <- rep(0, 10)
xyz <- matrix(NA, nr = 10, nc = 3)     # State of RNG
xyz[1, ] <- (a * saveSeed[2:4]) %% m  
u[1] <- sum(xyz[1, ]/m) %% 1           # Emit pseudo-random value
for(i in 2:10) {
...
## We should be able to recover the current value of the state.
xyz[10, ]
.Random.seed[2:4]
```
...
```{python}
#| eval: false
rng.normal(size = 3)     # Now generate based on chosen generator.
...
for parallel RNG, with more details given in the [SCF parallelization tutorial](https://computing.stat.berkeley.edu/tutorial-parallelization/parallel-python#5-random-number-generation-rng-in-parallel).
